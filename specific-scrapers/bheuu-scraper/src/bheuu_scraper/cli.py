"""
CLI entry point for the BHEUU scraper.

Usage:
    bheuu-scraper [OPTIONS]

    # Full run (requires DO_SPACES_* env vars):
    bheuu-scraper --site-config configs/bheuu.yaml

    # Dry run – fetch and parse, no Spaces upload, no state write:
    bheuu-scraper --dry-run

    # Incremental – only records published on or after 2025-01-01:
    bheuu-scraper --since 2025-01-01

    # Limit pages per section for a quick smoke test:
    bheuu-scraper --dry-run --max-pages 2
"""
...
from .pipeline import BHEUUPipeline, load_config
...
@click.option(
    ...
    default=None,
    metavar="YYYY-MM-DD",
    help="Skip records published before this date (inclusive lower bound).",
)
@click.option(
    "--max-pages",
    default=0,
    type=int,
    show_default=True,
    help="Limit number of API pages fetched per section (0 = unlimited).",
)
@click.option(
    "--dry-run",
    is_flag=True,
...
@click.option(
    ...
    show_default=True,
    envvar="BHEUU_DB_PATH",
    help="SQLite state database path.",
)
@click.option(
    "--manifest-dir",
    default="data/manifests/bheuu",
    show_default=True,
    envvar="BHEUU_MANIFEST_DIR",
...
def main(
    ...
    """BHEUU Government Site Scraper – archives public documents to DigitalOcean Spaces."""
