...
USER_AGENT = (
    "Mozilla/5.0 (compatible; dewan-johor-scraper/1.0; "
    "Python-Requests; +mailto:operator@example.com)"
)
REQUEST_TIMEOUT = 30  # seconds

_DEFAULT_ALLOWED_HOSTS: frozenset[str] = frozenset({"dewannegeri.johor.gov.my"})
...
def is_allowed_host(url: str, allowed_hosts: Optional[frozenset[str]] = None) -> bool:
...
class HTTPClient:
    ...
    def get(self, url: str, stream: bool = False) -> requests.Response:
        ...
        Always follows redirects (requests default). For WPDM download token
        URLs, the final resp.url will be the resolved file location.
        """
