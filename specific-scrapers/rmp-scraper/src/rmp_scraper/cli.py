"""
CLI entry point for the RMP scraper.

Usage:
    rmp-scraper [OPTIONS]

    # Full run (requires DO_SPACES_* env vars):
    rmp-scraper --site-config configs/rmp.yaml

    # Dry run – fetch and parse, no Spaces upload, no state write:
    rmp-scraper --dry-run

    # Incremental – only articles published on or after 2025-01-01:
    rmp-scraper --since 2025-01-01

    # Limit to 2 listing pages per section for a quick smoke test:
    rmp-scraper --dry-run --max-pages 2
"""
...
from .pipeline import RMPPipeline, load_config
...
@click.option(
    ...
    type=int,
    show_default=True,
    help="Limit listing pages fetched per seed URL (0 = unlimited).",
)
@click.option(
    "--dry-run",
    is_flag=True,
...
@click.option(
    "--manifest-dir",
    default="data/manifests/rmp",
    show_default=True,
    envvar="RMP_MANIFEST_DIR",
...
def main(
    ...
    """RMP Government Site Scraper – archives public documents to DigitalOcean Spaces."""
