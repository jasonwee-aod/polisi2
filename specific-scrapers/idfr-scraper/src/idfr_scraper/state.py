...
Deduplication strategy:
  1. Pre-fetch:  if canonical_url already exists AND ETag/Last-Modified unchanged → skip.
  2. Post-fetch: if sha256 already in DB → reuse existing gcs_uri, skip re-upload.
"""
...
class StateStore:
    ...
    def get_gcs_uri_by_sha256(self, sha256: str) -> Optional[str]:
        """Return an existing gcs_uri for content already uploaded, or None."""
        cur = self.conn.execute(
            "SELECT gcs_uri FROM documents WHERE sha256 = ? AND gcs_uri IS NOT NULL LIMIT 1",
            (sha256,),
        )
        row = cur.fetchone()
        return row["gcs_uri"] if row else None
    ...
    def upsert_record(self, record: Record) -> None:
        ...
        self.conn.execute(
            ...
            INSERT INTO documents
                (record_id, canonical_url, sha256, gcs_uri,
