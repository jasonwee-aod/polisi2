"""
CLI entry point for the IDFR scraper.

Usage:
    idfr-scraper [OPTIONS]

    # Full run (requires GCS_BUCKET env var and GOOGLE_APPLICATION_CREDENTIALS):
    idfr-scraper --site-config configs/idfr.yaml

    # Dry run – fetch and parse, no GCS upload, no state write:
    idfr-scraper --dry-run

    # Incremental – only documents published on or after 2024-01-01:
    idfr-scraper --since 2024-01-01

    # Limit to 5 listing pages for quick smoke test:
    idfr-scraper --dry-run --max-pages 5
"""
...
from .pipeline import IDFRPipeline, load_config
...
@click.option(
    ...
    type=int,
    show_default=True,
    help="Limit number of listing pages fetched (0 = unlimited; mainly used for smoke tests).",
)
@click.option(
    "--dry-run",
    is_flag=True,
...
@click.option(
    "--manifest-dir",
    default="data/manifests/idfr",
    show_default=True,
    envvar="IDFR_MANIFEST_DIR",
...
def main(
    ...
    """IDFR Government Site Scraper – archives public documents to GCS."""
