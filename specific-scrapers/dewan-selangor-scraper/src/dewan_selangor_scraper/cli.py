"""
CLI entry point for the Dewan Selangor scraper.

Usage:
    dewan-selangor-scraper [OPTIONS]
    ...
    dewan-selangor-scraper --dry-run --max-pages 3
"""
...
from .pipeline import DewanSelangorPipeline, load_config
...
@click.option(
    "--site-config",
    default="configs/dewan_selangor.yaml",
    show_default=True,
...
@click.option(
    ...
    show_default=True,
    envvar="DEWAN_SELANGOR_DB_PATH",
    help="SQLite state database path.",
)
@click.option(
    "--manifest-dir",
    default="data/manifests/dewan-selangor",
    show_default=True,
    envvar="DEWAN_SELANGOR_MANIFEST_DIR",
...
def main(
    ...
    """Dewan Selangor Government Site Scraper – archives public documents to GCS."""
    ...
    pipeline = DewanSelangorPipeline(
        config=config,
        state=state,
        archiver=archiver,
        http=http,
